...
def predict_from_datamodule(model, datamodule, rgb=(3, 2, 1), index=None, labels=None, plot=True):
    ...
    if plot:
        # plot the image
        plot_rgb(img,rgb=rgb, title=labels[label] if labels is not None else "Label: " + str(label))
    ...
    print("Predicted: ", labels[predicted_class] if labels is not None else "Pred: " + str(predicted_class))
...
if __name__ == '__main__':
    ...
    datamodule = UCMercedDataModule(root=root, batch_size=64, num_workers=4, download=True)
    input_channels = 3
    ...
    # predict
    img, label, pred, predicted_class = predict_from_datamodule(resnet,
                                                                datamodule,
                                                                labels=LABELS,
                                                                rgb=(0,1,2))
